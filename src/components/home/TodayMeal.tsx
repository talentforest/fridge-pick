...
export default function TodayMeal({ hasHeader = false }: TodayMealProps) {
  ...
  const [mainMenu, ...restMainMenuList] = todayMealList.filter(
    ({ meal }) => meal.mealType === 'main',
  );

  const sideMenuList = todayMealList.filter(({ meal }) => meal.mealType === 'side');

  const restMenuList = [...restMainMenuList, ...sideMenuList];
        ...
        <View className="flex-row justify-between gap-x-3">
          ...
          {restMenuList.length > 0 && (
            <View className="w-[31%] gap-y-3">
              {restMenuList.slice(0, 2).map((item) => (
